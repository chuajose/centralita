...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 **/
	public function hash_password_db_admin($id, $password, $use_sha1_override=FALSE)
	{
		if (empty($id) || empty($password))
		{
			return FALSE;
	 ...
	 * @author Mathew
	 **/
	public function login_admin($identity, $password, $remember=FALSE)
	{
		$this->trigger_events('pre_login');
		...
		$query = $this->db->select('email, id, password, active')
		...
		if ($query->num_rows() === 1)
		{
			...
			$password = $this->hash_password_db_admin($user->id, $password);
	...
	public function exist_user($id = FALSe)
	{
	...
	public function exist_user_company($id = FALSe)
	{

		$this->db->limit(1);
		$this->db->where('users_companies.user_id', $id);
		$this->response = $this->db->get('users_companies');
		if ($this->response->num_rows()==1)
		{
			$company = $this->response->row();
