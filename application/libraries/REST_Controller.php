...
abstract class REST_Controller extends CI_Controller {
    ...
    public function response($data = NULL, $http_code = NULL, $continue = FALSE)
    {
        ...
        elseif ($data !== NULL)
        {
            if ($this->userId) 
            {
                ...
                //echo "device".$this->deviceId."|";
                //Añado los datos del token
                $std       = new stdClass();
                $std->data =$this->userId;
                $std->device =$this->deviceId;
                $std->iat  = time();
                $std->exp  = time() + config_item('rest_time_expired_token');
                $jwt       = JWT::encode($std, '');
            }
            elseif($this->adminId)
            {
                $std       = new stdClass();
                $std->data =$this->adminId;
                $std->admin =$this->adminId;
                $std->iat  = time();
                $std->exp  = time() + config_item('rest_time_expired_token');
                $jwt       = JWT::encode($std, '');
            }
    ...
    protected function _parse_patch()
    {
        ...
        {
            if($this->_query_args) $this->_patch_args = $this->_query_args;
            else{
            // If no filetype is provided, then there are probably just arguments
